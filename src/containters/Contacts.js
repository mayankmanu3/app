...
export default class Contacts extends Component {
  ...
  componentDidUpdate = () => {
    if (this.state.refreshContacts) {
      ...
      this.props.updated_contacts(true);
    }
  ...
  getContacts = () => {
    ...
    if (contacts !== null) {
      ...
      if (currentUser !== null) {
        let getIndex = contacts.findIndex(
    ...
    return (
      <div className='container mt-5 pt-3 pl-lg-3 pl-xs-0 ml-lg-4 ml-xs-1'>
        <div className='d-flex flex-row mt-3 justify-content-start align-items-center'>
          <i className='fa fa-address-book idcard text-white' />
          <h3 className='d-flex flex-column pl-3'>
            <span className='heading mt-2'>Contacts</span>
          </h3>
        ...
        >
          <AddContact
            ...
            updateContactStatus={(status) =>
              this.setState({ updateContact: status })
            }
            refreshContacts={(status) =>
              this.setState({ refreshContacts: status })
            }
          />
        </div>

        {contacts.length === 0 && (
          <h3 className='ml-lg-5 nocontacts'>No Contacts Found!</h3>
        )}
        {contacts.length > 0 && (
          <ContactList
            contacts={contacts}
            ...
            updateContactStatus={(status) =>
              this.setState({ updateContactUser: status })
            }
